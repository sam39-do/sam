...
- 模式：`C` 或页面按钮切换生存/创造。生存模式每 20 秒有 20% 概率触发 12 秒创造能量。
- 改良：地图扩大到旧 3D 版 20 倍，玩家和怪物加入方块碰撞，降低穿墙问题。
- 建造/收集：左键挖掘掉落方块，靠近自动收集；右键放置当前方块。
- 火山：新大地图随机生成火山，20 秒喷发一次，熔岩命中会造成 10 秒持续伤害。
- 画面：本地 Three.js、电影色调、高质量阴影、Q 版圆润角色、小地图、生物表情和 10 倍粒子特效。
- 生命：主角最高生命调整为 100。
